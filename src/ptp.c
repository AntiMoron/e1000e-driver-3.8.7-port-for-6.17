...
/* New kernels expect an adjfine callback which takes scaled ppm; provide a
 * small wrapper that converts scaled ppm to parts-per-billion and calls the
 * legacy implementation which accepts ppb.
 */
static int e1000e_phc_adjfine(struct ptp_clock_info *ptp, long scaled_ppm)
{
	long ppb = scaled_ppm_to_ppb(scaled_ppm);
	return e1000e_phc_adjfreq(ptp, (s32)ppb);
}

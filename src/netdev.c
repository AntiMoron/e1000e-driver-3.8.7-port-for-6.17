...
static void e1000e_systim_reset(struct e1000_adapter *adapter)
{
	...
	if (info->adjfine) {
		/* restore the previous ptp frequency delta (adjfine uses scaled ppm)
		 * Convert stored ppb delta back to scaled ppm roughly: scaled_ppm = ppb * 2^13 / 125
		 */
		long scaled = (long)adapter->ptp_delta * (1 << 13) / 125;
		ret_val = info->adjfine(info, scaled);
	} else {
...
#endif
{
	...
	if (adapter->flags & FLAG_HAS_HW_TIMESTAMP) {
		/* New kernel uses const pointer to cyclecounter in read signature */
		adapter->cc.read = (u64 (*)(struct cyclecounter *))e1000e_cyclecounter_read;
...
{
	struct e1000_adapter *adapter = container_of(t, struct e1000_adapter, phy_info_timer);
...
{
	struct e1000_adapter *adapter = container_of(t, struct e1000_adapter, watchdog_timer);

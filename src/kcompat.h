...
/* PCIe error reporting helpers for older kernels */
...
/* NOTE: Do NOT provide driver symbol stubs for hwtstamp ioctls here.
 * The driver implements these when HAVE_HW_TIME_STAMP is available; if
 * needed, the build will disable those features via compile-time flags.
 */
#include <linux/io.h>
...
#ifndef strlcpy
static inline size_t __kc_strlcpy2(char *dst, const char *src, size_t siz)
{
...
#define strlcpy(dst, src, siz) __kc_strlcpy2((dst), (src), (siz))
#endif
#include <linux/tcp.h>
...
/* Indicate availability of hardware timestamp support for modern kernels */
#define HAVE_HW_TIME_STAMP

/* Provide conversion helper for ART -> system counter value when kernel
 * doesn't expose convert_art_to_tsc. Use clocksource id for x86 ART.
 */
#include <linux/timekeeping.h>
#ifndef convert_art_to_tsc
static inline struct system_counterval_t convert_art_to_tsc(u64 art)
{
	struct system_counterval_t val;
	val.cycles = art;
	val.cs_id = CSID_X86_ART;
	val.use_nsecs = false;
	return val;
}
#endif
...
/* Provide strlcpy for kernels that don't expose it */
#ifndef HAVE_STRLCPY
#include <linux/string.h>
static inline size_t __kc_strlcpy(char *dst, const char *src, size_t siz)
{
	size_t len = strlen(src);
	if (siz) {
		size_t copy = (len >= siz) ? siz - 1 : len;
...
#define strlcpy(dst, src, siz) __kc_strlcpy((dst), (src), (siz))
#endif
...
/* Ensure from_timer is available on all target kernels */
#ifndef from_timer
#define from_timer(var, callback_timer, timer_fieldname) \
	container_of(callback_timer, typeof(*var), timer_fieldname)
#endif

/* del_timer_sync may be a macro in newer kernels; provide a safe fallback */
#ifndef del_timer_sync
static inline void del_timer_sync(struct timer_list *timer)
{
	/* best-effort fallback to del_timer if del_timer_sync is not available */
	del_timer(timer);
}
#endif
